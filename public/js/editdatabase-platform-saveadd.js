...
function submitsave() {
    ...
    for (let i = 0; i < array.length; i++) {
        ...
        if (element.querySelector('[name="platformname"]').value=='') {
    ...
    if (flag==1) {
        ...
        xhrzx.open("POST", `/editdatabase/saveaddplatform`);
...
function returnarraystatussave(array) {
    return array.map(function(element) {
        return `
        <tr>
            <td class="text-truncate"  style="max-width: 100px;" title="${element.platform}">
                ${element.platform}
            </td>
            <td>
...
function klikdeleteitem(index) {
    ...
    Swal.fire({
        icon:'warning',
        title: '',
        text:`Apakah anda ingin delete item ${dataarray.platform}?`,
        showDenyButton: true,
    ...
    if (result.isConfirmed) {
        ...
        //delete item
        deletefinish(dataarray.platform);
    }
    });
}//
...
function deletefinish(item) {
    ...
    xhrzx.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
    xhrzx.setRequestHeader('authorization', xi);
    xhrzx.send(JSON.stringify({namalengkap,username,tipe:'platform',item}));
    xhrzx.addEventListener("load", () => {
        var resdat = JSON.parse(xhrzx.responseText);
        console.log('resdat');
        console.log(resdat);
