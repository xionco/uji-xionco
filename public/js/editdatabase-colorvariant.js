...
document.addEventListener('DOMContentLoaded', function () {
    ...
    showtabel(dataarray.colorvariantdata);
...
socket.on('newadditemadminlist',function(datas){
    ...
    console.log(dataarray.dataadminlist);
    loadingbawahupdate(dataarray.colorvariantdata);
});
...
function renderItems() {
    ...
    newItem.innerHTML =`
        <hr>
        <b><p id="judulitem-${itemIndex}">Color ${itemIndex}</p></b>
        
        <div class="row">
            <div class="input-group input-group-sm mb-3">
                <span class="input-group-text" style="background-color: rgb(226, 221, 221);font-size: 12px;width: 120px;"><span class="text-danger">*&nbsp;</span> Color Variant</span>
                <input type="text" class="form-control" id="colorvariant-${itemIndex}" name="colorvariant" placeholder="Color variant Name" style="font-size: 12px;"  oninput="oninputdata()">
            </div>
...
function removeItem(itemId) {
    ...
    items.forEach((item, index) => {
        ...
        judulitem.innerHTML=`Color ${newId}`;
...
function oninputdata() {
    ...
    document.querySelectorAll('#itemsContainer > .partitems').forEach((itemCard,index) => {
        let colorvariant = itemCard.querySelector('[name="colorvariant"]').value;
...
$('#modaladdnewitem').on('hidden.bs.modal', function (e) {
    addoredit='add';
    document.getElementById(`modaladdnewitemlabel`).innerHTML="Add New Color Variant";
    ...
    document.querySelectorAll('#itemsContainer > .partitems').forEach((itemCard,index) => {
        itemCard.querySelector('[name="colorvariant"]').value='';
